...
def train(config, train_loader, model, loss_func, optimizer, epoch, output_dir, writer_dict):
    ...
    for i, (images, xc, yc, xt, yt, w, theta) in enumerate(train_loader):
        ...
        if config.MODEL.PREDICT_THETA:
            perf = evaluate_orientaion_theta(output.detach().cpu().numpy(),
                                             target_output.detach().cpu().numpy())
        else:
            perf = evaluate_orientaion_coords(output.detach().cpu().numpy(), 
                                             target_output.detach().cpu().numpy(),
                                             theta,
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            if not config.MODEL.PREDICT_THETA:
