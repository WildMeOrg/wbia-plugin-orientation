...
def plot_rotated(input_images, coords_gt, coords_pred, theta_gt, theta_pred,
                 prefix, output_dir, min_rows=2, max_cols=4, max_rows=4):
    ...
    for r in range(nrows):
        for c in range(ncols):
            ...
            image_rotated = transform.rotate(images_un[r*ncols+c],
                                             angle=degrees_gt,
                                             center=coords_gt[r*ncols+c, :1])
            ax[r, 2*c].imshow(image_rotated)
            ax[r, 2*c].set_title('GT Rotated by {:.0f} deg'.
                                 format(degrees_gt))
            ...
            ax[r, 2*c+1].set_title('Preds Rotated by {:.0f} deg'.
                                   format(degrees_pred))
